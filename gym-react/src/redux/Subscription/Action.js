...
export const getAllSubscription = (jwt) => {
  return async (dispatch) => {
    dispatch({ type: types.GET_ALL_SUBSCRIPTION_REQUEST });
    try {
      ...
      console.log("users subscription ",response.data)
        ...
        console.log(error)
      dispatch({
        type: types.GET_ALL_SUBSCRIPTION_FAILURE,
        payload: error.message,
      });
    }
  };
...
export const createSubscription = ({planType}) => {
  return async (dispatch) => {
    ...
    try {
      const response = await api.post("/api/subscriptions", null, {
        params: {
          planType: planType,
        },
      });
